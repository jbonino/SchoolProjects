...
void redraw(){
	...
	if(rotAngle>0) rotAngle-=2;
	if(rotAngleCounter<0) 
		rotAngleCounter+=2;
	glPopMatrix();
...
void drawsquare (picinfo *p, int row, int col){
	...
	if(p->flag){
		if(rotAngle>0){
			if(rotatingside==0){
				if(p->r>0) glRotatef(-rotAngle,1,0,0);
				else if(p->t>0) glRotatef(-rotAngle,0,1,0);
				else if(p->b>0) glRotatef(rotAngle,0,1,0);
				else if(p->l>0) glRotatef(rotAngle,1,0,0);
				else glRotatef(rotAngle,0,0,1);
			}
			if(rotatingside==1){
				if(p->r>0) glRotatef(-rotAngle,1,0,0);
				else if(p->t>0) glRotatef(rotAngle,1,0,0);
				else if(p->b>0) glRotatef(rotAngle,1,0,0);
				else if(p->l>0) glRotatef(rotAngle,1,0,0);
				else glRotatef(rotAngle,0,0,1);
			}
			if(rotatingside==2){
			...
			if(rotatingside==4){
				if(p->r>0) glRotatef(-rotAngle,0,1,0);
				else if(p->t>0) glRotatef(-rotAngle,0,1,0);
				else if(p->b>0) glRotatef(-rotAngle,0,1,0);
				else if(p->l>0) glRotatef(-rotAngle,0,1,0);
				else glRotatef(rotAngle,0,0,1);
			}
			if(rotatingside==5){
				if(p->r>0) glRotatef(rotAngle,0,1,0);
			...
			//counter
			if(rotatingside==0){
				if(p->r>0) glRotatef(-rotAngleCounter,1,0,0);
				else if(p->t>0) glRotatef(-rotAngleCounter,0,1,0);
				else if(p->b>0) glRotatef(rotAngleCounter,0,1,0);
				else if(p->l>0) glRotatef(rotAngleCounter,1,0,0);
				else glRotatef(rotAngleCounter,0,0,1);
			}
			if(rotatingside==1){
				if(p->r>0) glRotatef(-rotAngleCounter,1,0,0);
				else if(p->t>0) glRotatef(rotAngleCounter,1,0,0);
				else if(p->b>0) glRotatef(rotAngleCounter,1,0,0);
				else if(p->l>0) glRotatef(rotAngleCounter,1,0,0);
				else glRotatef(rotAngleCounter,0,0,1);
			}
			if(rotatingside==2){
			...
			if(rotatingside==4){
				if(p->r>0) glRotatef(-rotAngleCounter,0,1,0);
				else if(p->t>0) glRotatef(-rotAngleCounter,0,1,0);
				else if(p->b>0) glRotatef(-rotAngleCounter,0,1,0);
				else if(p->l>0) glRotatef(-rotAngleCounter,0,1,0);
				else glRotatef(rotAngleCounter,0,0,1);
			}
			if(rotatingside==5){
				if(p->r>0) glRotatef(rotAngleCounter,0,1,0);
...
void swapOutside(int b[], int r[], int t[], int l[]){
	...
	sticker[b[6]] [ b[4] ][ b[5] ]->b=1;
	sticker[b[6]] [ b[2] ][ b[3] ]->b=1;
	sticker[b[6]] [ b[0] ][ b[1] ]->b=1;

	sticker[l[6]][l[4]][l[5]]->l=1;
	sticker[l[6]][l[2]][l[3]]->l=1;
	sticker[l[6]][l[0]][l[1]]->l=1;

	sticker[t[6]][t[4]][t[5]]->t=1;
	sticker[t[6]][t[2]][t[3]]->t=1;
	sticker[t[6]][t[0]][t[1]]->t=1;

	sticker[r[6]][r[4]][r[5]]->r=1;
	sticker[r[6]][r[2]][r[3]]->r=1;
	sticker[r[6]][r[0]][r[1]]->r=1;
